...
- `Adress` (string) — адрес проживания PhoneNumber
...
- Связь многие-к-одному с таблицей `Reader_category` (многие пользователи могут относиться к одной категории)
- Связь один-ко-многим с таблицей `Loans` (один пользователь может иметь много выдач)
...
- `User_id` (int, FK → [User]) — читатель
- `Book_id` (int, FK → [Book]) — книга

**Связи:**

- Связь многие-к-одному с таблицей `User` (каждая выдача принадлежит одному пользователю)
...
- Связь многие-к-одному с таблицей `User` (одна категория может быть присвоена многим пользователям)
...
- Кнопка "Читатели" — открывает UserWindow для просмотра списка читатаелей
...
- Кнопка "Удалить" — удаляет выбранную книгу после подтверждениия
- Кнопка "Закрыть" — закрывает данную форму
- Кнопка "Редактировать" — открывает EditBookWindow для редактирования книги
- Кнопка "Поиск" — поиск книги по параматрам
- Кнопка "Сброс" — сброс параметров поиска

#### Форма 3: UserWindow (Список читателей)

**Назначение:** Просмотр и управление каталогом книг библиотеки

**Элементы управления:**

- DataGrid - отображение списка читатаелей из базы данных
- StackPanel — панель отображения основных кнопок 
- TextBlock —  поле для заголовка формы
...
- Кнопка "Добавить читателя" — открывает EditUserWindow для добавления нового читателя
- Кнопка "Читательский билет" — открывает форму UserTicketWindow
- Кнопка "Удалить" — удаляет выбранного читателя после подтверждениия
- Кнопка "Закрыть" — закрывает данную форму
- Кнопка "Редактировать" — открывает EditUserWindow для редактирования информации о читателе
- Кнопка "Поиск" — поиск читателя по параматрам
- Кнопка "Сброс" — сброс параметров поиска

#### Форма 4: UserTicketWindow (Читательский билет)
...
- Кнопка "Сброс" — сброс паметров поиска
- Кнопка "Поиск" — поиск книги по параматрам
- Кнопка "Закрыть" — закрывает данную форму
...
- Кнопка "Оформить возварт" — открывает PaymentWindow для оплаты возврата
- Кнопка "Закрыть" — закрывает данную форму
...
#### Форма 9: EditUserWindow (Редактор читателя)
...
**Секция "Читатели":**
- Кнопка "Читатели" → `UserWindow` (просмотр списка читателей)
...
- Кнопка "Добавить читателя" → `EditUserWindow` (режим создания нового читателя)
- Кнопка "Редактировать" (выбран читатель в списке) → `EditUserWindow` (режим редактирования читателя)
- Кнопка "Удалить" (выбран читатель) → диалог подтверждения → удаление читателя, остаёмся в UserWindow
- Кнопка "Читательский билет" (выбран читатель) → `UserTicketWindow` (просмотр билета читателя)
...
**Секция "Навигация":**
- Кнопка "Закрыть" → возврат на `UserWindow`
...
**Секция "Навигация":**
- Кнопка "Закрыть" → возврат на `UserTicketWindow`
...
- Кнопка "Оплатить" → обработка платежа → сообщение об успехе/ошибке
  - При успешной оплате выдачи: возврат на `UserTicketWindow` с обновлением статуса
  - При успешной оплате возврата: возврат на `UserTicketWindow` с обновлением статуса
...
#### Из EditUserWindow
**Секция "Сохранение":**
- Кнопка "Сохранить" → сохранение данных читателя → возврат на `UserWindow` (обновление списка читателей)

**Секция "Навигация":**
- Кнопка "Закрыть" → возврат на `UserWindow` без сохранения

### Логика переходов между основными операциями:

1. **Полный цикл выдачи книги:**
   `MainWindow` → `UserWindow` → `UserTicketWindow` → `IssueBookWindow` → `PaymentWindow` → `UserTicketWindow`

2. **Полный цикл возврата книги:**
   `MainWindow` → `UserWindow` → `UserTicketWindow` → `ReturnBookWindow` → `PaymentWindow` → `UserTicketWindow`

3. **Управление каталогом книг:**
   `MainWindow` → `BookWindow` → `EditBookWindow` → `BookWindow`

4. **Управление читателями:**
   `MainWindow` → `UserWindow` → `EditUserWindow` → `UserWindow`
