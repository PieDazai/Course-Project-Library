...
Данная система предназначена для автоматизации бизнес-процессов проката книг в библиотеке. Она позволяет учитывать выдачу и возврат книг, вести учет книг и читателей, рассчитывать стоимость аренды и залога, а также управлять системой штрафов.
---
...
- **Описание:** Добавление нового читателя в систему. Пользователь вводит ФИО, контактные данные (телефон, email), дату рождения. Система автоматически генерирует уникальный номер читательского билета.
- **Входные данные:** ФИО, телефон, email, дата рождения.
- **Результат:** Новый читатель добавлен в базу данных, выдан номер билета.

#### Функция 2: Добавление книги
...
#### Функция 3: Выдача книги с оплатой

- **Описание:** Оформление выдачи книги читателю с оплатой. Пользователь выбирает книгу и читателя, система определяет залоговую стоимость, генерирует уникальный номер выдачи и устанавливает дату выдачи, а также ожидаемую дату возврата. Пользователь проводит оплату залога и выдает книгу.
- **Входные данные:** Читатель, книга, платежные реквизиты.
- **Результат:** Добавление записи о выдаче книги читателю в базу данных.

#### Функция 4: Прием книги с оплатой

- **Описание:** Прием книги от читателя. Пользователь выбирает читателя и возвращаемую книгу, система фиксирует факт возврата и рассчитывает итоговую сумму на основе стоимости проката за фактический срок, учитывая штрафы за повреждения, если они были указаны. Возвращается залоговая стоимость книги и производится оплата проката. Пользователь принимает книгу у читателя.
- **Входные данные:** Читатель, книга, штрафы,  платежные реквизиты.
- **Результат:** Изменение записи в базе данных о возврате и запись итоговой стоимости проката.

#### Функция 5: Редактирование читателя

- **Описание:** Изменение данных существующего читателя. Пользователь выбирает читателя из списка и может изменить его ФИО, контактные данные, дату рождения и адрес проживания.
- **Входные данные:** Выбранный читатель, новые данные.
- **Результат:** Обновление информации о читателе в базе данных.

#### Функция 6: Редактирование книги

- **Описание:** Изменение информации о существующей книге. Пользователь выбирает книгу из каталога и может изменить название, автора, жанр, стоимость проката, залоговую стоимость, количество экземпляров.
- **Входные данные:** Выбранная книга, новые данные.
- **Результат:** Обновление информации о книге в базе данных.
...
#### Форма 10: StatisticsWindow (Окно статистики)

**Назначение:** Просмотр статистики по возрастным категориям читателей и доходам по месяцам

**Элементы управления:**
- TabControl — переключение между вкладками статистики
- PlotView (OxyPlot) — отображение круговой диаграммы распределения возрастных категорий
- PlotView (OxyPlot) — отображение столбчатой диаграммы доходов по месяцам
- DataGrid — таблица с данными о доходах по месяцам
- ListView — детализация статистики по категориям
- DatePicker — выбор периода для фильтрации статистики доходов
- StackPanel — панель отображения основных кнопок

**Действия:**
- Кнопка "Применить фильтр" — фильтрация статистики доходов по выбранному периоду
- Кнопка "Сбросить" — сброс фильтров
- Кнопка "Закрыть" — закрывает окно статистики
...
- Вкладка "Возрастные категории" → вкладка с возрастными категориями читатаелей
- Вкладка "Доходы по месяцам" → вкладка с доходами по месяцам
